// Collect apical leading edge tracking information for curvature calculation
...
for (folder=0;folder<FolderList.length;folder++) {
	...
	newDir=path+FolderList[folder]+"Tracking"+File.separator;
	...
	for (file=0;file<FileList.length;file++){
		if (endsWith(FileList[file],'.zip')) {
			if (startsWith(FileList[file],"Leading-edge_s")) {
				// Open tracking
				roiManager("open",path+FolderList[folder]+FileList[file]);
				nROIs=roiManager("count");
				// Detect name and open reslice
				NameEnd=substring(FileList[file],indexOf(FileList[file],"dge_")+4,lengthOf(FileList[file])-4);
